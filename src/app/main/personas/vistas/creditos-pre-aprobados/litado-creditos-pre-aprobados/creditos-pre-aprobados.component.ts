...
@Component({
  selector: 'app-creditos-pre-aprobados',
  templateUrl: './creditos-pre-aprobados.component.html',
  styleUrls: ['./creditos-pre-aprobados.component.scss']
})
export class CreditosPreAprobadosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._creditosPreAprobadosService.obtenerInformacion(this.usuario.id).subscribe(info => {
  ...
  guardarRegistro() {
    ...
    this._creditosPreAprobadosService.guardarInformacion(this.informacion).subscribe(info => {
  ...
  continuar(value) {
    if (value == 7) {
      this._creditosPreAprobadosService.crearCredito(this.solicitarCredito).subscribe((info) => {
        this.proceso = value;
      });
    } else {
      this.proceso = value;
